...
router.put('/cancel/:id',async (req, res) => {
    try {
      ...
      if (!canceledTrip) return res.status(404).json({ message: 'Booking not found' });
...
router.post("/book/:tripId",checkForAuthentication,async (req,res) => {
    try {
        ...
        return res.status(500).json({ message: "Failed to book trip", error: error.message });
    }
